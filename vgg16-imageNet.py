...
class vgg16:
    def __init__(self, imgs, label, class_num, learning_rate,keep_prob,count, weights=None,sess=None):
        self.imgs = imgs
        self.label = label
        self.count = count
...
def train(img_dir,classes,X_train_final,y_train_final,X_valid,y_valid,count):
    ...
    sess = tf.Session()
    vgg_name = vgg16(imgs=imgs, label=label, class_num=len(classes), learning_rate=Learning_rate,keep_prob=Keep_prob,
                     count=count,weights='vgg16_weights.npz',sess=sess)
